import type { LoginModel } from '~/fixture/mock'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  watchImmediate(access_token, token => last_login_time.value = token ? et.value : -1)
  ...
  async function login(
    ...
    const res = await $weilaFetch<LoginModel>('/corp/web/login', {
      body: params,
    })
  ...
  function logout(isDeleteLoginRecord?: boolean) {
    access_token.value = ''
    refresh_token.value = ''
    last_login_time.value = -1
    expires_in.value = -1

    if (isDeleteLoginRecord)
      accountHistoryRecord.value.clear()

    toast({ description: '账号登出' })
    router.push('/login')
  }
  ...
  const isOutdate = computedWithControl(
    ...
    () => isLogin.value && (et.value - last_login_time.value > expires_in.value - EXPIRES_BUFFER),
  )
  ...
  const refreshToken = useThrottleFn(async () => {
    try {
      ...
      const { $v1 } = useNuxtApp()
      const { errcode, data } = await $v1<any>('/sessions/refresh', {
        body: {
