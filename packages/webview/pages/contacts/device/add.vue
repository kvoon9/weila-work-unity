...
const addDeviceSchema = z.object({
  verify_code: z.string().max(20).nonempty(),
...
const { handleSubmit, errors, defineField } = useForm({
  validationSchema: toTypedSchema(addDeviceSchema),
})

const [verify_code, verify_codeAttrs] = defineField('verify_code')
...
const { mutate: addDevice, isPending } = useMutation({
  mutationFn: async (values: z.infer<typeof addDeviceSchema>) => {
...
<template>
  <div p8>
    <form flex flex-col space-y-2 @submit.prevent="onSubmit">
      <label label for="verify_code">设备验证码</label>
      <input v-bind="verify_codeAttrs" v-model="verify_code" input type="text" name="verify_code">
