...
const { refetch } = useBusinessStaffList({ sid: sid.value })
...
async function fetchStaff() {
  try {
    ...
    const result = await useBusinessStaffList({ sid: sid.value }).refetch()
...
// 使用解构并重命名变量以避免lint警告
const { mutate: delStaff, isPending: isDeleting } = useDelBusinessStaff({
  onSuccess() {
    toast({ title: '删除成功' })
