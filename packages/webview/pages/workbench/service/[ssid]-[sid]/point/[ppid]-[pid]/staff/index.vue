...
const { data: point } = useBusinessPoint(sid.value, pid.value)

watchEffect(() => {
  if (point.value)
    appHead.value = `${point.value?.name}`
})
...
const { data: _staffList, refetch: refetchStaffList } = useGetBusinessPointStaffList({ pid: pid.value })
...
const { mutate: addMembers, isPending: isAddingMembers } = useAddBusinessPointStaffs({
  onSuccess() {
